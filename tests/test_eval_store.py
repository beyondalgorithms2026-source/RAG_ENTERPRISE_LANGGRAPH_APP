...
def test_compute_metrics_handles_empty_report():
    metrics = compute_metrics(
        {"total": 0, "passed": 0, "failed": 0, "manual_review": 0, "rows": []},
        settings=_settings(),
    )
...
def test_eval_store_save_list_get_latest(tmp_path):
    ...
    first = build_eval_run_summary(
        SAMPLE_REPORT, settings=_settings(), eval_run_id="eval-20260101-000000-aaaa"
    )
    second = build_eval_run_summary(
        SAMPLE_REPORT, settings=_settings(), eval_run_id="eval-20260102-000000-bbbb"
    )
