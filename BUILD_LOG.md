...
Actual through D10: D1-D2 completed Tue 1 Sep (8h), D3-D5 completed Wed 2 Sep
...
Mon 7 Sep (~1.7h), followed by D10 (~2.6h). Cumulative actual is ~30.8h.
B004-MIN: 6 days, 36h, baseline Sat 6 Sep
B004 FULL: 11 days, 66h, expected Tue 8 Sep (baseline Sat 12 Sep; D8-D10 were brought
forward and the remaining date now depends on the owner-recorded D11 walkthrough; the original plan's
...
- Governed app demo: https://rag-enterprise-governance-demo.onrender.com/app
...
- [x] D10 Mon 7 Sep (brought forward from Wed 9 Sep) — deploy front end
...
- [ ] D11 Tue 8 Sep (expected; owner recording dependent) — walkthrough

## Verified facts — correct these against reality, never the reverse

- App tests: **114** as of D10 (was 109; +4 config tests and +1 public-demo boundary test). All offline. Verified from
...
### D10 — Mon 7 Sep 2026 (completed; brought forward from Wed 9 Sep)
...
- Added four one-click scenarios: supported answer, unsupported refusal, high-risk compliance
...
- The original three-ask-per-minute setting was unsuitable behind the governance service:
  multiple visitor questions share one Render egress IP and a single orchestrated recovery
  can use several backend calls. With explicit owner approval it is now 12 calls/minute;
  four generation-free dry runs returned HTTP 200. The OpenAI project's USD 1 monthly hard
  enforcement remains the absolute spend boundary.
- The unsupported preset now uses zero additional recovery steps. Its first grounded call
  already returns `Not found in provided sources.` with no citations; allowing recovery
  caused generic company terms to select irrelevant evidence and weakened a clear refusal.
- The human-review preset uses a supported public compliance question. The evidence check
  passes, then the answer is withheld as `pending_approval`; public visitors cannot decide it.

**Broke**

- The first live answerable run encountered a transient Render HTTP 502 while the backend
  redeployed. A direct retry returned HTTP 200 and the supported 26-day answer; subsequent
  app runs passed. No answer was fabricated during the failure.
- The first unsupported-preset wording recovered an irrelevant remote-working passage and
  labelled it `needs_review`. The bounded preset now ends as `not_found` with zero citations.
- The first review preset referred to an internal grievance source unavailable to anonymous
  visitors. It routed correctly but had no public evidence, so it was replaced by the
  supported public gift-policy compliance question.
...
- Public app `/app` redirected and loaded anonymously with all four presets and no login.
- Answerable preset: `verified`, correct 26-day answer, complete evidence, citation and
  working full-source link to the 933-byte Annual Leave Policy.
- Unsupported revenue preset: `not_found`, no grounded answer, zero citations.
- Compliance preset: supported evidence is checked and the answer is withheld as
  `pending_approval`; visitor approval decisions remain disabled.
- Restricted salary prompt: `not_grounded`, no grounded answer, zero citations.
- Phone viewport 390×844: four presets collapsed to one column, navigation wrapped and no
  horizontal overflow was present.
- Public mutation/operator checks: approval request, eval run, red-team run, direct model
  chat and demo-proof endpoints all returned HTTP 403.
- Final GitHub Actions runs: starter `34124299817` green in 50s; app `34125225204`
  green in 31s. Local app suite remained 114/114 passed.

**Hours**

- ~2.6h after D9 on Mon 7 Sep. Cumulative actual: ~30.8h of the 66h plan.

**Commits**

- Starter: `25e59d6` (serve public-demo sources from stored chunks)
- Starter: `5d6db57` (owner-approved governed-call rate)
