"""Generate provisional, public-safe v2 evidence from one preserved run artifact."""
...
def build_status(
    ...
    return {
        ...
        "approval_statement": "One evidence run; not calibration or an approved v2 baseline.",
        "approved_baseline": {"id": "northwind-openai-v1", "case_count": 25},
        ...
        "operations": {
            "transport_failures": int(report.get("transport_failures", 0)),
            "grader_failures": int(report.get("grader_failures", 0)),
            "other_infrastructure_failures": int(report.get("other_infrastructure_failures", 0)),
            "infrastructure_failures": int(report.get("infrastructure_failures", 0)),
        },
        "governance": {
        ...
        "limitations": {
            ...
            "statements": [
                ...
                "This is one reproducible evidence run, not ten-run calibration.",
...
<!doctype html><meta charset="utf-8"><title>Provisional v2 quality evidence</title>
...
<h1>Provisional B004 v2 quality evidence</h1>
<p class="note">{esc(status["approval_statement"])} The approved 25-case v1 baseline remains unchanged.</p>
...
<p>Source SHA-256: <code>{esc(status["source_artifact"]["sha256"])}</code></p>"""
