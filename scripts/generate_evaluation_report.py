...
Nothing on the page is written by hand. Evaluation and red-team numbers are read from
files produced by actual runs; the test result is measured by running the suite:
...
<main>
  ...
  <p>{e(scenario_count)} red-team scenarios: {e(defended)} defended, {e(failed_rt)} failed,
     {e(requires_backend)} labelled requires_backend by design.
  ...
  <p>The corpus is synthetic and generated by <code>corpus/generate_corpus.py</code> in
     the data-layer repository, so the source of every document is public. Every question and its expected answer is
